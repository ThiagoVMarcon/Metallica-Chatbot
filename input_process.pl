...
is_composed_response(["Im doing good, how about you?"], R) :- write("Cliff: "), adicionar_espaco(["Im doing good, how about you?"],Z),atomics_to_string(Z, O), write(O),write("\n"), write("User: "),read_line_to_codes(user_input, Ascii_imp),string_codes(Sla, Ascii_imp), read_split(Sla, 3, [], R).
...
process_Pattern(["I am good"], ["Good to hear!"]).
